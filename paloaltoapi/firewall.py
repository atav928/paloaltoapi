...
from paloaltoapi.device_groups.objects.services import FirewallServices
...
class Firewall:
    ...
    def __init__(self, device, username=None, passwd=None,
        ...
        self.api_key = key if key else self.get_key(
            username, passwd, update=False)
        self.system = None
    ...
    def get_version(self):
        ...
        try:
            ...
            # print(xmldict)
            self.sw_version = xmldict['result']['system'].get(
                'sw-version', None)
            try:
            ...
            self.devicename = xmldict['result']['system'].get(
                'devicename', None)
            self.model = xmldict['result']['system'].get('model', None)
            self.serial = xmldict['result']['system'].get('serial', None)
            self.ip_address = xmldict['result']['system'].get(
                'ip-address', None)
            self.default_gateway = xmldict['result']['system'].get(
    ...
    def check_jobid(self, jobid: str) -> Dict[str, Any]:
        """Check Status of Job"""
        resp = check_job(self.device, self.api_key,
                         jobid=jobid, verify=self.certstore)
        result = format_resp(resp)
        return result
    ...
    #    self.Services = FirewallServices()
